...
fun RouteHandlersProvider.V1.Collections.Cards.getCardById(
    ...
    // Return 404 if if there is no collection with a corresponding id
    ...
    // Return 400 if the card does not belong to the appropriate collection
    if (card.collectionId != collectionId) {
        val message = mapOf("message" to "The card with \"id\" property \"$cardId\" does not belong to the collection")
        call.respond(status = HttpStatusCode.NotFound, message = message)
        return@getCollectionByIdHandler
    }
